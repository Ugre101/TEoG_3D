...
namespace Map
{
    public sealed class TerrainBorder : MonoBehaviour
    {
        ...
        void BuildBorder()
        {
            ...
            (north != null ? north : CreateCube(out north)).localPosition = new Vector3(data.bounds.center.x - moveBorderIn, 0, 0);
            (south != null ? south : CreateCube(out south)).localPosition = new Vector3(-data.bounds.center.x + moveBorderIn, 0, 0);
            (east != null ? east : CreateCube(out east)).localPosition = new Vector3(0, 0, data.bounds.center.z - moveBorderIn);
            (west != null ? west : CreateCube(out west)).localPosition = new Vector3(0, 0, -data.bounds.center.z + moveBorderIn);
            SetScales(data);
        }
