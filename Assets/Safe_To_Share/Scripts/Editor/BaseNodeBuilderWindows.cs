...
namespace CustomClasses.Editor {
    public abstract class BaseNodeBuilderWindows<T, N> : EditorWindow
        where T : BaseEditorCanvasObject<N> where N : BaseEditorCanvasNode {
        ...
        protected static void DrawNodeConnection(BaseEditorCanvasObject<N> canvasObject, N node) {
            foreach (var childNode in canvasObject.GetChildNodes(node)) {
