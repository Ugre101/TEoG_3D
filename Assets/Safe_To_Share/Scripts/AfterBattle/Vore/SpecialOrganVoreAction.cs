...
namespace Safe_To_Share.Scripts.AfterBattle.Vore {
    ...
    public sealed class SpecialOrganVoreAction : VoreAction {
        ...
        public override bool CanUse(BaseCharacter giver, BaseCharacter receiver) {
            if (!giver.SexualOrgans.Containers.TryGetValue(organType, out var container) ||
                !container.BaseList.Any())
                return false;
            ...
            return onePreyOnly
                ...
                : container.BaseList.Any(baseOrgan =>
                    VoreSystemExtension.CanOrganVore(giver, baseOrgan, receiver, organType));
        }
