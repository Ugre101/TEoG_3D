...
namespace AvatarStuff
{
    public partial class CharacterAvatar
    {
        ...
        public void GetRandomBodyMorphs(BaseCharacter character)
        {
            ...
            {
                int res = UgreTools.Levenshtein.Compute(body.Title, faceShape.Title);
                if (res < min)
                {
                    hit = faceShape;
                    min = res;
        ...
        public struct BodyShapes
        {
            ...
            {
                SkinnedMeshRenderer[] meshRenderers = renderers as SkinnedMeshRenderer[] ?? renderers.ToArray();
            ...
            {
                int index = shapeName.LastIndexOf("male_", StringComparison.Ordinal);
                string cleanedName = shapeName.Substring(index + 5);
