...
namespace GameUIAndMenus.Menus.EssenceMenu
{
    public class EssenceOptionsToggles : MonoBehaviour
    {
        ...
        public void Setup(Player parPlayer)
        {
            ...
            SetupToggle(player.Essence.EssencePerks.OfType<HeightEssencePerk>().Any(), giveHeight, player.Essence.EssenceOptions.GiveHeight);

            SetupTransmute();
            SetupGenderMorph();
        }

        private void SetupGenderMorph()
        {
            ...
            if (hasGenderMorphPerk)
            {
                ...
                void SetMode(int arg) => player.Essence.EssenceOptions.MorphPartnerToGender = UgreTools.IntToEnum(arg, GenderMorph.MorphToGender.Disabled);
            }
            genderMorhpOption.gameObject.SetActive(hasGenderMorphPerk);
        }

        private void SetupTransmute()
        {
            bool hasTransMutePerk = player.Essence.EssencePerks.OfType<AutoTransMuteEssencePerk>().Any();
            if (hasTransMutePerk)
            {
                transmuteOption.SetupTmpDropDown(player.Essence.EssenceOptions.TransmuteTo, SetMode, DrainEssenceType.Both);
                void SetMode(int arg) => player.Essence.EssenceOptions.TransmuteTo = UgreTools.IntToEnum(arg, DrainEssenceType.None, DrainEssenceType.Both);
            }
