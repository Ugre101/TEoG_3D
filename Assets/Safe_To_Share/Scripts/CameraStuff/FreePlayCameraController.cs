...
namespace Safe_To_Share.Scripts.CameraStuff {
    public sealed class FreePlayCameraController : MonoBehaviour {
        ...
        IEnumerator TargetElevation(float change) {
            var loop = true;
            while (loop) {
                var y = Mathf.Clamp(target.localPosition.y + change, 0.1f, 3f);
        ...
        public void OnLook(InputAction.CallbackContext ctx) {
            ...
            if (ThirdPersonCameraSettings.AlwaysLook) {
                if (player.velocity.FlatVel().magnitude >= 0.3f)
                    LockCursor();
                else if (player.velocity.FlatVel().magnitude < 0.3f && !holding)
                    UnlockCursor();
            }
            ...
            if (mouseLookInput.y != 0)
                TiltTarget(
                    ThirdPersonCameraSettings.InvertHorizontalAxis.Enabled ? -mouseLookInput.y : mouseLookInput.y);
