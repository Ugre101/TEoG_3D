...
namespace Character.RelationShipStuff
{
    ...
    public class RelationsShips
    {
        ...
        public Dictionary<int, RelationShip> RelationShipsDict =>
            relationShipsDict ??= RelationShips.ToDictionary(r => r.WithID);
        ...
        public void IncreaseAffectionTowards(BaseCharacter towards, float by) =>
            GetRelationShipWith(towards.Identity.ID).Affection += by;

        public void DecreaseAffectionTowards(BaseCharacter towards, float by) =>
            GetRelationShipWith(towards.Identity.ID).Affection -= by;

        public void IncreaseSubmissivenessTowards(int id, float by) => GetRelationShipWith(id).Affection += by;

        public void IncreaseSubmissivenessTowards(BaseCharacter towards, float by) =>
            IncreaseSubmissivenessTowards(towards.Identity.ID, by);

        public void IncreaseDomesivenessTowards(BaseCharacter towards, float by) =>
            GetRelationShipWith(towards.Identity.ID).Submission -= by;
    }
}