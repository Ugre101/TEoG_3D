...
namespace Character.VoreStuff {
    ...
    public class VoreOrgan {
        ...
        public void SetStretch(float capacity) {
            var stomachPreys = VoredCharacters.CurrentPreyTotalWeight(PreysIds);
        ...
        public float DigestTick(float toDigest, float stretch, Action<Prey> digested, bool predIsPlayer) {
            float v = 0;
            for (var index = PreysIds.Count; index-- > 0;)
        ...
        float DigestPreyAndReturnAmount(float toDigest, Action<Prey> digested, float v, int index, bool predIsPlayer) {
            var preysId = PreysIds[index];
            if (!VoredCharacters.PreyDict.TryGetValue(preysId, out var prey))
                return v;
            var preyDigest = prey.Digest(toDigest, predIsPlayer);
            v += preyDigest;
            if (preyDigest < toDigest) {
                ...
                var shouldTrigger = Random.value / timesPleadedThisSession > 0.8f;
        ...
        public void TickHour(int ticks = 1) {
            foreach (var preysId in PreysIds) {
