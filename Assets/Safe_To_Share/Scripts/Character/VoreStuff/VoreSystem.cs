...
namespace Character.VoreStuff {
    ...
    public class VoreSystem {
        ...
        VoreOrganDigestionMode GetDigestionMode(VoreType type) =>
            type switch {
        ...
        public bool VoreTick(BaseCharacter pred, bool predIsPlayer, int ticks) {
            ...
            for (var i = 0; i < ticks; i++) {
                foreach (var vorePerk in Level.OwnedPerks)
        ...
        void RegurgitateStomachPrey(BaseCharacter pred) {
            var toReg = FindLeastDigested(stomach.PreysIds);
        ...
        bool TickOrganContainer(BaseCharacter pred,
                                KeyValuePair<SexualOrganType, VoreOrganDigestionMode> organDigestionMode,
                                bool predIsPlayer) {
            if (!pred.SexualOrgans.Containers.TryGetValue(organDigestionMode.Key, out var container))
                return false;
            var change = false;
            foreach (var baseOrgan in container.BaseList.TakeWhile(OrganHasPreys))
        ...
        void HandleSpecialSexOrganDigestion(BaseCharacter pred, SexualOrganType key, BaseOrgan baseOrgan,
                                            bool predIsPlayer) {
            for (var index = baseOrgan.Vore.SpecialPreysIds.Count; index-- > 0;) {
        ...
        bool HandleOrganRegurgitation(BaseCharacter pred, string organName, BaseOrgan baseOrgan) {
            if (baseOrgan.Vore.Stretch < 1f)
                return false;
            var toReg = FindLeastDigested(baseOrgan.Vore.PreysIds);
        ...
        void PleasureDigestion(BaseCharacter pred, List<int> preyIds) {
            if (!CanPleasurePreys)
                return;
            foreach (var preysId in preyIds)
        ...
        Prey FindLeastDigested(IEnumerable<int> preyList) {
            ...
            float least = 0;
            foreach (var p in VoredCharacters.GetPreys(preyList)) {
                var percentDigested = p.Body.Weight / p.StartWeight;
