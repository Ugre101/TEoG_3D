...
namespace Items {
    ...
    public class Inventory {
        ...
        async Task<bool> LowerItemAmount(string itemGuid, int amount = 1) {
            ...
            if (invItem.Count <= 0)
                return false;
            var tot = invItem.Sum(i => i.Amount);
            if (tot < amount)
                return false;
        ...
        public bool UseLoadedItem(Item item, Vector2 pos, int times = 1) {
            var itemOnPos = GetItemOnPos(pos);
            if (itemOnPos == null)
        ...
        public bool MoveItemInsideInventory(InventoryItem toMove, Vector2 newPos, out InventoryItem oldItem) {
            var hadItem = TryGetItemOnPos(newPos, out oldItem);
            if (hadItem)
        ...
        public bool AddInventoryItemToPos(ref InventoryItem toAdd, Vector2 toPos, out InventoryItem oldItem) {
            var hadItem = TryGetItemOnPos(toPos, out oldItem);
        ...
        public bool MoveToAnotherInventory(Inventory moveTo, ref InventoryItem toMove, Vector2 toPos,
                                           out InventoryItem oldItem) {
            oldItem = null;
            var oldPos = toMove.Position;
            Items.Remove(toMove);
        ...
        public void Load(InventorySave toLoad) {
            ...
            foreach (var loadItem in toLoad.items)
