...
namespace Character.BodyStuff {
    public static class BodyExtensions {
        public static float Train(Body body, float intensity = 1f) {
            var muscle = body.Muscle;
            var musclePercent = muscle.BaseValue / body.Weight;
            var demisingReturn = 1f - musclePercent;
            var gain = body.Height.BaseValue * 0.005f * demisingReturn * intensity;
            muscle.BaseValue += gain;
            return gain;
        }

        public static float BurnFatHour(this Body body, int ticks = 1, float extraActivityFactor = 0f) {
            var daily = body.Weight * 10 + body.Height.Value * 6.25f;
            daily *= body.FatBurnRate.Value + extraActivityFactor; // you are moving around
            var perHour = daily / 24f;
            var inFat = perHour / 9000f;
        ...
        static string FatDesc(float fatRatio) =>
            fatRatio switch {
        ...
        static string FitnessDesc(float fitnessRatio) =>
            fitnessRatio switch {
                > 1.2f => "athletic",
                > 1.1f => "defined",
                < 0.8f => "pudgy",
        ...
        public static float GetFatRatio(this Body body) {
            ...
            var fatRatio = body.FatWeight / body.Weight;
            fatRatio = fatRatio * 100 / avgFat;
            return fatRatio;
        }

        public static void GainPercentFat(Body body, float fatGainPercent) {
            var currentPercent = body.Fat.BaseValue / body.Weight;
            var goalPercent = currentPercent + Mathf.Clamp(fatGainPercent, 0, 1f);
        ...
        public static string HeightDesc(this BaseCharacter character) {
            var avgHeight = character.RaceSystem.Race.AverageHeight;
            var ratio = character.Body.Height.Value / avgHeight;
            StringBuilder desc =
        ...
        static string HeightRatioDesc(float ratio) =>
            ratio switch {
        ...
        public static float GetMuscleRatio(this Body body) {
            ...
            // 20kg at 160cm = 0%
            // 50kg at 160cm = 100%
            var muscleRatio = body.Muscle.Value / 100f;
