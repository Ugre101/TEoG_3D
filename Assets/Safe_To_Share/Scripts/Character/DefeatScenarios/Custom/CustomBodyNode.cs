...
namespace Character.DefeatScenarios.Custom
{
    ...
    public class CustomBodyNode : CustomLoseScenarioNode
    {
        ...
        {
            if (!activePlayerActor.Body.BodyStats.TryGetValue(bodyType, out BodyStat playerBody))
                return;
        ...
        void HandlePermChange(BaseCharacter activeEnemyActor, BodyStat playerBody)
        {
            if (permChange == 0)
                return;
            if (!transfer || !activeEnemyActor.Body.BodyStats.TryGetValue(bodyType, out BodyStat enemyBody))
            {
