...
namespace Character.GenderStuff {
    public static class GenderSettings {
        ...
        static Dictionary<Gender, string> SavedGenders =>
            savedGenders ??= new Dictionary<Gender, string> {
        ...
        public static Gender GetGender(BaseCharacter baseCharacter) {
            var hasBalls = baseCharacter.SexualOrgans.Balls.HaveAny();
            var hasDick = baseCharacter.SexualOrgans.Dicks.HaveAny();
            var hasVagina = baseCharacter.SexualOrgans.Vaginas.HaveAny();
            var hasBoobs = baseCharacter.SexualOrgans.Boobs.Biggest > BoobThreesHold;
            if (hasDick) {
                if (hasVagina)
