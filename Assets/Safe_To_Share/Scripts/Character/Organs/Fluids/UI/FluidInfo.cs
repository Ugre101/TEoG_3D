...
namespace Character.Organs.Fluids.UI {
    public sealed class FluidInfo : MonoBehaviour {
        ...
        public void UpdateFluid(BaseOrgansContainer baseOrgansContainer, float bodyHeight) {
            gameObject.SetActive(baseOrgansContainer.Fluid.Recovery.Value > 0 ||
                                 baseOrgansContainer.Fluid.CurrentValue > 0);
            slider.maxValue = baseOrgansContainer.Fluid.Value;
            slider.value = baseOrgansContainer.Fluid.CurrentValue;
            var fluidInText = SexualOrgansExtensions.FluidAmountInText(baseOrgansContainer.FluidCurrent, bodyHeight);
