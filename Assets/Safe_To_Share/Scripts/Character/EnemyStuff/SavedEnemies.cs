...
namespace Character.EnemyStuff
{
    public static class SavedEnemies
    {
        ...
        {
            foreach (Enemy e in Enemies.Values.SelectMany(list => list))
                e.NullifyRemoved();
        ...
        static void CreateEnemy(int id, IReadOnlyList<EnemyPreset> presets, ICollection<Enemy> newEnemies, Islands islands)
        {
