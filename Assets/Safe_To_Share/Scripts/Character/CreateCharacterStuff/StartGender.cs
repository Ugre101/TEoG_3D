...
namespace Character.CreateCharacterStuff {
    ...
    public class StartGender {
        ...
        Gender GetGender() {
            ...
            var totalWeight = allowedGenders.Sum(g => g.weight);
            List<GenderBias> weightedList = new();
            var currWeight = 0;
            foreach (var bias in allowedGenders) {
                currWeight += bias.weight;
                weightedList.Add(new GenderBias(currWeight, bias.gender));
            }

            var index = rng.Next(totalWeight);
            foreach (var bias in weightedList.Where(bias => bias.weight > index))
                return bias.gender;
        ...
        public void SetGender(BaseCharacter character, Islands islands) {
            ...
            var masc = data.essenceData.Masc.Current;
            if (masc > 0)
                character.GainMasc(masc);
            else if (masc < 0)
                character.LoseMasc(masc, new ChangeLog());
            var femi = data.essenceData.Femi.Current;
            if (femi > 0)
