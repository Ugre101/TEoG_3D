...
namespace Safe_To_Share.Scripts.Movement.HoverMovement.Modules {
    ...
    public sealed class SwimmingModule : BaseMoveModule {
        ...
        [SerializeField, Range(float.Epsilon, 1f),]
        float swimAtPercentSubmerged = 0.5f;

        [SerializeField, Range(float.Epsilon, 0.2f),]
        float swimDepthMargin = 0.1f;
        ...
        float OffFromTargetBy() {
            var test = capsule.Height * (swimAtPercentSubmerged + swimDepthMargin);
        ...
        public override void OnMove(Vector3 force) {
            ...
            var diff = capsule.YMin - SeaBottom();
