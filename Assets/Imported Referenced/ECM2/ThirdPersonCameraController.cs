...
namespace Movement.ECM2.Source.Common
{
    /// <summary>
    ...
    public class ThirdPersonCameraController : MonoBehaviour, IColliderFilter
    {
        ...
        [Tooltip("The object that the camera wants to move with (e.g. Its target"), SerializeField,]
        Transform _follow;
        ...
        float _followMinDistance;

        [Tooltip("The maximum distance to Follow target."), SerializeField,]
        float _followMaxDistance;

        [Tooltip("The change in rotation per second (Deg / Sec) when looking around."), SerializeField,]
        float _lookRate;

        [Tooltip("The change in follow distance per second."), SerializeField,]
        float _zoomRate;

        [Space(15f), Tooltip("Should the cursor be locked?"), SerializeField,]
        bool _lockCursor;

        [Tooltip("Determines if the vertical axis should be inverted."), SerializeField,]
        bool _invertLook;

        [Tooltip("The horizontal sensitivity while using Mouse input, higher values cause faster movement."),
         SerializeField,]
        float _mouseHorizontalSensitivity;

        [Tooltip("The vertical sensitivity while using Mouse input, higher values cause faster movement."),
         SerializeField,]
        float _mouseVerticalSensitivity;

        [Tooltip("The horizontal sensitivity while using a Controller, higher values cause faster movement."),
         SerializeField,]
        float _controllerHorizontalSensitivity;

        [Tooltip("The vertical sensitivity while using a Controller, higher values cause faster movement."),
         SerializeField,]
        float _controllerVerticalSensitivity;

        [Space(15f), Tooltip("Should clamp pitch rotation?"), SerializeField,]
        bool _clampPitchRotation;
        ...
        [Header("Obstacles"), Tooltip("Camera will avoid obstacles on these layers."), SerializeField,]
        LayerMask cameraCollisionFilter = 1;

        [Tooltip("Specifies how close the camera can get to obstacles."), SerializeField,]
        float _cameraRadius;

        [Tooltip("Colliders to be ignored as obstacles."), SerializeField,]
        List<Collider> _ignoredColliders = new List<Collider>();
        ...
        public virtual void LockCameraBehindTarget()
        {
            if (!_isCursorLocked)
                lockToRearOfTarget = true;
        }

        public virtual void UnLockCameraBehindTarget() => lockToRearOfTarget = false;

        /// <summary>
        ///     Is the cursor locked ?
        /// </summary>
        public virtual bool IsCursorLocked() => _isCursorLocked;
        ...
        /// </summary>
        protected virtual void AddYawInput(float value) => _yaw = MathLib.Clamp0360(_yaw + value);
        ...
        /// </summary>
        public virtual void ZoomAtRate(float value) => followDistance -= value * zoomRate * Time.deltaTime;

        /// <summary>
        ///     Set the camera current orientation.
        /// </summary>
        protected virtual void UpdateCameraRotation() => transform.rotation = Quaternion.Euler(_pitch, _yaw, 0.0f);
        ...
        protected virtual void UpdateCameraPosition()
        {
            ...
            var prevTargetPos = Time.deltaTime >= 0 ? _prevFollowPosition : targetPos;

            Quaternion transformRotation = transform.rotation;
            var dampedTargetPos = Quaternion.Inverse(transformRotation) * (targetPos - prevTargetPos);
            dampedTargetPos = prevTargetPos + transformRotation * dampedTargetPos;

            Vector3 cameraForward = transform.forward;

            int hitCount = CollisionDetection.SphereCast(dampedTargetPos, cameraRadius, -cameraForward,
                followDistance, cameraCollisionFilter, QueryTriggerInteraction.Ignore, out RaycastHit closestHit, Hits,
                this, 0.002f);
        ...
        protected virtual void RotateBehindTarget()
        {
            if (!lockToRearOfTarget)
                return;
            float targetRotationAngle = follow.eulerAngles.y;
            float currentRotationAngle = transform.eulerAngles.y;

            // Stop rotating behind if not completed 
            if (Math.Abs(targetRotationAngle - currentRotationAngle) > float.Epsilon)
                _yaw = Mathf.LerpAngle(currentRotationAngle, targetRotationAngle, rotateBehindDamp * Time.deltaTime);
        }
