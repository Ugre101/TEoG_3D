...
namespace Movement.ECM2.Source.Components
{
    ...
    public struct GroundDistance
    {
        ...
        /// <summary>
        ///     True if the hit found a valid walkable ground using a raycast (rather than a sweep test, which happens when the
        ///     sweep test fails to yield a walkable surface).
        /// </summary>
    ...
    ///     The CharacterMovement component allows you to easily perform movement constrained by collisions without having to
    ///     deal with its inherent difficulty.
    ///     This is based in the collide-and-slide algorithm, a collision detection algorithm capable of detect 'future'
    ///     collisions and gracefully
    ...
    public class CharacterMovement : MonoBehaviour, IColliderFilter
    {
        ...
        /// </summary>
        bool IColliderFilter.Filter(Collider otherCollider) => IgnoreCollider(otherCollider);
        ...
        [Space(15f), Tooltip("Allow to constrain the Character so movement along the locked axis is not possible."),
         SerializeField,]
        PlaneConstraint _constrainToPlane;

        [Space(15f), Tooltip("The Character's capsule collider radius."), SerializeField,]
        float _capsuleRadius;

        [Tooltip("The Character's capsule collider height"), SerializeField,]
        float _capsuleHeight;

        [Space(15f), Tooltip("The maximum angle (in degrees) for a walkable slope."), SerializeField,]
        float _slopeLimit;

        [Tooltip("The maximum height (in meters) for a valid step (up to Character's height)."), SerializeField,]
        float _stepOffset;
        ...
        [Tooltip("Determines if a Character should collide with triggers."), SerializeField,]
        bool _collideWithTriggers;

        [Space(15f), Tooltip("Determines the layers to be considered as walkable ground."), SerializeField,]
        LayerMask _groundMask;

        [Tooltip("Determines the maximum length of the cast.\n" +
                 "As a rule of thumb, configure it to your Character's collider radius."), SerializeField,]
        float _groundProbingDistance;

        #endregion

        #region FIELDS

        protected readonly RaycastHit[] _hits = new RaycastHit[kMaxHitCount];
        protected readonly Collider[] _overlaps = new Collider[kMaxOverlapCount];

        protected readonly HashSet<Collider> _ignoredColliders = new();

        protected GroundHit _lastGroundHitResult;
        protected GroundHit _groundHitResult;

        protected int _movementHitCount;
        protected readonly MovementHit[] _movementHits = new MovementHit[kMaxHitCount];

        protected readonly List<PhysicsVolume> _volumes = new();

        protected readonly List<RigidbodyHit> _rigidbodyHits = new();
        ...
        Vector3 _capsuleTopCenter;
        Vector3 _capsuleBottomCenter;

        Vector3 _planeConstraintNormal = Vector3.zero;
        ...
        ICharacterMovementCallbacks _callbackTarget;
        ...
        /// <summary>
        ///     Allow a Character to perch on the edge of a surface if the horizontal distance from the Character's position to the
        ///     edge is closer than this.
        ///     Note that we still enforce stepOffset to start the step up, this just allows the Character to hang off the edge or
        ///     step slightly higher off the ground.
        /// </summary>
        ...
        /// <summary>
        ///     When perching on a ledge, add this additional distance to stepOffset when determining how high above a walkable
        ///     floor we can perch.
        /// </summary>
        ...
        /// <summary>
        ///     Allow a Character to unperch from the edge of a surface if the horizontal distance from the Character's position to
        ///     the edge is greater than this.
        ///     Note that characters will not fall off if they are within stepOffset of a walkable surface below.
        /// </summary>
        ...
        /// </summary>
        protected static void MakeCapsule(float radius, float height, out Vector3 center, out Vector3 bottomCenter,
        ...
        protected virtual bool IgnoreCollider(Collider otherCollider) =>
            otherCollider == capsuleCollider || _ignoredColliders.Contains(otherCollider);
        ...
        /// <summary>
        ///     Casts a ray, from point origin, in direction direction, of length distance, against specified colliders (by
        ///     layerMask) in the Scene.
        /// </summary>
        protected bool Raycast(Vector3 origin, Vector3 direction, float distance, int layerMask,
            ...
            CollisionDetection.Raycast(origin, direction, distance, layerMask, triggerInteraction,
                out closestHit, _hits, this) > 0;

        /// <summary>
        ///     Casts a capsule against specified colliders (by layerMask) in the Scene and returns detailed information on what
        ///     was hit.
        /// </summary>
            ...
            Collider[] results, QueryTriggerInteraction triggerInteraction = QueryTriggerInteraction.Ignore)
        {
            ...
            return CollisionDetection.OverlapCapsule(bottom, top, capsuleRadius, layerMask, triggerInteraction, results,
                this);
        }
        ...
        ///     Check the given capsule against the physics world and return all overlapping colliders.
        /// </summary>
        protected int OverlapCapsule(Vector3 characterPosition, Quaternion characterRotation, float radius,
            float height,
        ...
        public bool SweepTest(Vector3 direction, float distance, int layerMask, out RaycastHit closestHit,
            QueryTriggerInteraction triggerInteraction = QueryTriggerInteraction.Ignore) =>
            CapsuleCast(position, rotation, direction, distance, layerMask, out closestHit, _hits, 0.002f,
        ...
        /// </summary>
        public Collider[] OverlapTest(int layerMask, out int overlapCount,
            QueryTriggerInteraction triggerInteraction = QueryTriggerInteraction.Ignore)
        {
        ...
        /// </summary>
        public bool CheckCapsule(float radius, float height,
            QueryTriggerInteraction triggerInteraction = QueryTriggerInteraction.Ignore)
        {
            int overlapCount = OverlapCapsule(position, rotation, radius, height, groundMask & collisionMask, _overlaps,
                triggerInteraction);
        ...
        public virtual void SetCapsuleDimensions(float radius, float height)
        {
            ...
            MakeCapsule(_capsuleRadius, _capsuleHeight, out _capsuleCenter, out _capsuleBottomCenter,
        ...
        /// </summary>
        public virtual bool IsConstrainedToPlane() => _constrainToPlane != PlaneConstraint.None;
        ...
        public virtual Vector3 ConstrainDirectionToPlane(Vector3 vector)
            => _constrainToPlane == PlaneConstraint.None ? vector : vector.projectedOnPlane(_planeConstraintNormal);

        /// <summary>
        ///     Is the Character constrained to walkable ground ?
        /// </summary>
        public virtual bool IsConstrainedToGround()
            => _isConstrainedToGround && _unconstrainedTimer <= 0.0f;
            ...
            float sweepDistance, float sweepRadius, out GroundDistance distanceResult, float rayLength = 0.0f)
        {
            ...
            int hitCount = CapsuleCast(characterPosition, characterRotation, sweepRadius, capsuleHeight, sweepDirection,
                sweepDistance + kContactOffset, sweepLayerMask, out RaycastHit sweepHitResult, _hits,
                kStepSweepBackstepDistance, sweepTriggerInteraction);

            if (hitCount > 0)
            {
                Vector3 positionOnGround =
                    characterPosition + sweepDirection * sweepDistance + characterUp * kContactOffset;
            ...
            if (rayLength > 0.0f)
            {
                bool hit = Raycast(characterPosition, sweepDirection, rayLength + kContactOffset, sweepLayerMask,
                    out RaycastHit raycastHitResult, sweepTriggerInteraction);
                if (hit)
                {
                    distanceResult = new GroundDistance
                    {
            ...
            Quaternion characterRotation, Vector3 inNormal)
        {
            ...
            return verticalComponent switch
            {
                > 0.01f => CapsuleHitLocation.Bottom,
                < -0.01f => CapsuleHitLocation.Top,
                _ => CapsuleHitLocation.Sides,
            };
        }

        /// <summary>
        ///     Attempts to detect a ledge at given position.
        ///     It is considered a ledge if found no ground, found no walkable ground, or found any ground farther than step
        ///     offset.
        /// </summary>
            ...
            ref RaycastHit inHitResult, out LedgeHit outLedgeHitResult)
        {
            ...
            const float rayLength = kMinHeightForLedge + kRaycastVerticalOffset;
            ...
            outLedgeHitResult = new LedgeHit
            {
                ...
                isOnEmptySide = isFarHitWalkable,
            ...
            Vector3 inPoint, Vector3 inNormal, out bool perchResult)
        {
            ...
            bool nearHit = Raycast(rayOrigin, rayDirection, actualRayLength, groundMask, out RaycastHit nearHitResult,
                triggerInteraction);
            ...
            Vector3 displacement, ref RaycastHit inHitResult, out StepHit outStepHitResult)
        {
            ...
            Vector3 sweepOrigin =
                MathLib.ProjectPointOnPlane(inHitResult.point, characterPosition, characterUp) +
                characterUp * stepOffset - projectedNormal * kRaycastHorizontalOffset;
            ...
            int hitCount = CapsuleCast(sweepOrigin, characterRotation, sweepDirection, sweepDistance + kContactOffset,
            ...
            int overlapCount = OverlapTest(positionOnStep, characterRotation, groundMask & collisionMask, _overlaps,
                triggerInteraction);
            if (overlapCount != 0)
                return;
            ...
            bool farHit = stepHitResult.collider.Raycast(new Ray(rayOrigin, rayDirection), out RaycastHit farHitResult,
                rayLength);
            bool isFarHitWalkable = farHit && Vector3.Dot(farHitResult.normal, characterUp) > _minSlopLimit;
            if (!isFarHitWalkable)
                return;
            ...
            rayOrigin = nearPoint + projectedNormal * (2.0f * kRaycastHorizontalOffset) +
                        characterUp * kRaycastVerticalOffset;

            rayLength = stepOffset + perchAdditionalHeight + kRaycastVerticalOffset;

            bool nearHit = Raycast(rayOrigin, rayDirection, rayLength, groundMask, out RaycastHit nearHitResult,
                triggerInteraction);
            bool isNearHitWalkable = nearHit && Vector3.Dot(nearHitResult.normal, characterUp) > _minSlopLimit;
            if (!isNearHitWalkable)
            {
                ...
                rayOrigin = stepHitResult.point + projectedNormal * (kRaycastHorizontalOffset * 2.0f) +
                            characterUp * kRaycastVerticalOffset;

                rayLength = stepOffset + perchAdditionalHeight + kRaycastVerticalOffset;

                nearHit = Raycast(rayOrigin, rayDirection, rayLength, groundMask, out nearHitResult,
                    triggerInteraction);
            ...
            if (isClimbable)
            {
                ...
                sweepOrigin = characterPosition + characterUp * Mathf.Max(0.0f,
                    Vector3.Dot(stepHitResult.point - characterPosition, characterUp));
                ...
                hitCount = CapsuleCast(sweepOrigin, characterRotation, sweepDirection, sweepDistance,
                    groundMask & collisionMask, out stepHitResult, _hits, kDisplacementSweepBackstepDistance,
                    triggerInteraction);
                ...
                else
                {
                    ...
                    Vector3 displacementToHit =
                        sweepDirection * stepHitResult.distance + stepHitResult.normal * kContactOffset;
                ...
                hitCount = CapsuleCast(sweepOrigin, characterRotation, sweepDirection, sweepDistance, groundMask,
            ...
            float sweepDistance, out GroundHit outGroundHitResult)
        {
            ...
            TestWalkableGround(positionOnGround, characterRotation, Vector3.zero, ref hitResult,
                out outGroundHitResult);
            {
        ...
        /// <summary>
        ///     Sweeps the capsule volume along its displacement vector, stopping at near hit point if collision is detected or
        ///     applies full displacement if not.
        ...
        protected virtual Vector3 ComputeMovementHit(Vector3 displacement, out MovementHit outMovementHitResult)
        {
            ...
            else
            {
                ...
                TestWalkableGround(_probingPosition, _probingRotation, displacement, ref hitResult,
                    out GroundHit groundHitResult);
                {
        ...
        protected virtual void CollideAndSlide(Vector3 displacement)
        {
            ...
            for (int i = 0, iteration = 0; iteration < kMaxMovementIterations && i < kMaxHitCount; ++i)
            {
                ...
                if (movementHitResult.hitWalkableGround && IsConstrainedToGround())
                {
                    ...
                    displacement =
                        _callbackTarget.ComputeCollisionResponseDisplacement(displacement, ref movementHitResult);
                    ...
                    switch (iteration)
                    {
                        case 0:
                            ...
                            displacement =
                                _callbackTarget.ComputeCollisionResponseDisplacement(displacement,
                                    ref movementHitResult);

                            velocity = _callbackTarget.ComputeCollisionResponseDisplacement(velocity,
                                ref movementHitResult);
                        ...
                        case 1:
                            ...
                            if (Vector3.Dot(oVel, nVel) <= 0.0f ||
                                Vector3.Dot(prevNormal, movementHitResult.normal) < 0.0f)
                            {
        ...
        public virtual void Move()
        {
            ...
            _rigidbody.velocity =
                ConstrainDirectionToPlane((_probingPosition - position) / Time.deltaTime + activePlatformVelocity);
        ...
        ///     This needed since we resolve future collisions, we cache hits and trigger after PhysX internal update, like regular
        ///     OnCollisionXXX events.
        /// </summary>
        ...
        {
            capsuleCollider.radius = capsuleRadius + kContactOffset;
            capsuleCollider.height = capsuleHeight + kContactOffset * 2.0f;
            ...
            int overlapCount = OverlapTest(_probingPosition, _probingRotation, overlapMask, _overlaps,
                triggerInteraction);
            if (overlapCount > 0)
                for (int i = 0; i < overlapCount; i++)
                {
                    ...
                    if (Physics.ComputePenetration(capsuleCollider, _probingPosition, _probingRotation,
                            overlappedCollider, overlappedColliderTransform.position,
        ...
        protected virtual void UpdatePhysicsVolume(Collider other)
        {
            ...
            if (other.ClosestPoint(characterCenter) == characterCenter)
            {
                ...
                if (other.gameObject.TryGetComponent(out PhysicsVolume newPhysicsVolume))
            ...
            else
            {
                ...
                if(other.gameObject.TryGetComponent(out PhysicsVolume newPhysicsVolume) && newPhysicsVolume == physicsVolume) 
        ...
        { 
            // Is a physics volume ?
            if (!other.TryGetComponent(out PhysicsVolume volume)) return;
        ...
        /// </summary>
        public virtual void SetCallbackTarget(ICharacterMovementCallbacks target) => _callbackTarget = target;

        /// <summary>
        ///     Removes the callback target.
        /// </summary>
        public virtual void RemoveCallbackTarget() => _callbackTarget = null;
        ...
        void OnTriggerEnter(Collider other) => OnOnTriggerEnter(other);

        void OnTriggerExit(Collider other) => OnOnTriggerExit(other);

        readonly WaitForFixedUpdate waitTime = new();
