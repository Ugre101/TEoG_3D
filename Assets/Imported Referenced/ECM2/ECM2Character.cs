using System;
using ECM2.Source.Helpers;
using ECM2.Source.Interfaces;
...
namespace Movement.ECM2.Source.Characters
{
    ...
    ///     - walking:  Walking on a surface, under the effects of friction, and able to "step up" barriers. Vertical velocity
    ///     is zero.
    ...
    ///     - OrientToMovement:            Rotates the Character towards the given input move direction vector, using
    ///     rotationRate as the rate of rotation change.
    ///     - OrientToCameraViewDirection: Rotates the Character towards the camera's current view direction, using
    ///     rotationRate as the rate of rotation change.
    ...
    ///     and has been designed for a vertically-oriented player representation that can walk, run, jump, fly, and swim
    ///     through the world.
    ///     This Character class serves as a robust platform to build your game mechanics on top of it, as this has been
    ///     developed with extensibility in mind, because in the end,
    ///     no one knows your game better than you!
    /// </summary>
    public class ECM2Character : MonoBehaviour, ICharacterMovementCallbacks
    {
        #region EDITOR EXPOSED FIELDS

        [Header("Rotation"), Tooltip("Change in rotation per second (Deg / s)."), SerializeField,]
        float _rotationRate;

        [Tooltip("Character's current rotation mode."), SerializeField,]
        RotationMode _rotationMode;

        [Header("Walking"), Tooltip("The maximum ground speed when walking.\n" +
                                    "Also determines maximum lateral speed when falling."), SerializeField,]
        float _maxWalkSpeed;

        [Tooltip("The ground speed that we should accelerate up to when walking at minimum analog stick tilt."),
         SerializeField,]
        float _minAnalogWalkSpeed;
        ...
        [Header("Sprinting"), Tooltip("The walk speed multiplier while sprinting."), SerializeField,]
        float _sprintSpeedMultiplier;

        [Tooltip("The walk acceleration multiplier while sprinting."), SerializeField,]
        float _sprintAccelerationMultiplier;

        [Header("Crouching"), Tooltip("The maximum ground speed when walking and crouched."), SerializeField,]
        float _maxWalkSpeedCrouched;

        [Tooltip("The Character's capsule height while crouched."), SerializeField,]
        float _crouchedHeight;

        [Tooltip("If true, allows to jump while crouched (uncrouch character)."), SerializeField,]
        bool _crouchedJump;
        ...
        [Range(0.0f, 1.0f), Tooltip("When falling, amount of lateral movement control available to the Character.\n" +
                                    "0 = no control, 1 = full control at max acceleration."), SerializeField,]
        float _airControl;

        [Header("Jumping"), Tooltip("The max number of jumps the Character can perform."), SerializeField,]
        int _jumpMaxCount;
        ...
        float _jumpMaxHoldTime;

        [Tooltip("How early before hitting the ground you can trigger a jump (in seconds)."), SerializeField,]
        float _jumpPreGroundedTime;

        [Tooltip("How long after leaving the ground you can trigger a jump (in seconds)."), SerializeField,]
        float _jumpPostGroundedTime;

        [Space(15f), Tooltip("The gravity applied to this Character."), SerializeField,]
        Vector3 _gravity;

        [Header("Swimming"), Tooltip("If True, this Character is capable to Swim or move through fluid volumes."),
         SerializeField,]
        bool _canSwim;

        [Tooltip("The maximum swimming speed."), SerializeField,]
        float _maxSwimSpeed;
        ...
        float _swimmingFriction;

        [Tooltip("Water buoyancy ratio. 1 = Neutral Buoyancy, 0 = No Buoyancy."), SerializeField,]
        float _buoyancy;

        [Header("Flying"), Tooltip("The maximum flying speed."), SerializeField,]
        float _maxFlySpeed;
       ...
       [SerializeField] RootMotionController _rootMotionController;
       ...
       [SerializeField] Transform _cameraTransform;
        ...
        ///     Higher values allow faster changes in direction.
        ///     If useSeparateBrakingFriction is false, also affects the ability to stop more quickly when braking (whenever
        ...
        /// <summary>
        ///     Friction (drag) coefficient applied when braking (whenever Acceleration = 0, or if Character is exceeding max
        ///     speed).
        ...
        public Transform cameraTransform
        {
            get
            {
                if (isCameraTransformNull && _camera != null)
        ...
        protected virtual Vector2 GetMovementInput() =>
            inputVec; // movementInputAction?.ReadValue<Vector2>() ?? Vector2.zero;
        ...
        public delegate void LaunchedEventHandler(Vector3 launchVelocity, bool overrideVerticalVelocity,
            bool overrideLateralVelocity);
        ...
        /// </summary>
        protected virtual void OnLaunched(Vector3 launchVelocity, bool overrideVerticalVelocity,
            bool overrideLateralVelocity) =>
        ...
        {
            if (newPhysicsVolume && newPhysicsVolume.waterVolume) // Entering a water volume
                Swim();
            else if (IsInWater() && newPhysicsVolume == null) // Left a water volume
                StopSwimming();
        ...
        void ICharacterMovementCallbacks.OnMove() => OnMove();

        Vector3 ICharacterMovementCallbacks.ComputeCollisionResponseDisplacement(Vector3 displacement,
            ref MovementHit movementHit) =>
            ComputeCollisionResponseDisplacement(displacement, ref movementHit);

        bool ICharacterMovementCallbacks.CanStepUp(ref RaycastHit hitResult) => CanStepUp(ref hitResult);
        ...
        bool ICharacterMovementCallbacks.ShouldMoveCharacterWhenStandingOn(Rigidbody otherRigidbody) =>
            ShouldMoveCharacterWhenStandingOn(otherRigidbody);

        bool ICharacterMovementCallbacks.ShouldRotateCharacterWhenStandingOn(Rigidbody otherRigidbody) =>
            ShouldRotateCharacterWhenStandingOn(otherRigidbody);
        ...
        /// </summary>
        public virtual CharacterMovement GetCharacterMovement() => characterMovement;

        /// <summary>
        ///     Return the RootMotionController or null is not found.
        /// </summary>
        public virtual RootMotionController GetRootMotionController() => rootMotionController;

        /// <summary>
        ///     Return the Character's current PhysicsVolume, null if none.
        /// </summary>
        public virtual PhysicsVolume GetPhysicsVolume() => characterMovement.physicsVolume;

        /// <summary>
        ///     The Character's current position.
        /// </summary>
        public virtual Vector3 GetPosition() => characterMovement.position;

        /// <summary>
        ///     Sets the Character's position.
        ///     This complies with the interpolation resulting in a smooth transition between the two positions in any intermediate
        ///     frames rendered.
        /// </summary>
        public virtual void SetPosition(Vector3 position) => characterMovement.position = position;

        /// <summary>
        ///     Sets the Character's position.
        ///     Unlike SetPosition, this cause an instant change in position without any interpolation.
        /// </summary>
        public virtual void TeleportPosition(Vector3 position) => transform.position = position;

        /// <summary>
        ///     The Character's current rotation.
        /// </summary>
        public virtual Quaternion GetRotation() => characterMovement.rotation;

        /// <summary>
        ///     Sets the Character's current rotation.
        ///     This complies with the interpolation resulting in a smooth transition between the two rotations in any intermediate
        ///     frames rendered.
        /// </summary>
        public virtual void SetRotation(Quaternion rotation) => characterMovement.rotation = rotation;

        /// <summary>
        ///     Sets the Character's rotation.
        ///     Unlike SetRotation, this cause an instant change in rotation without any interpolation.
        /// </summary>
        public virtual void TeleportRotation(Quaternion rotation) => transform.rotation = rotation;

        /// <summary>
        ///     Sets the Character's current rotation mode:
        ///     - None:                        Disables character's rotation.
        ///     - OrientToMovement:            Orient the Character towards the given input move direction vector, using
        ///     rotationRate as the rate of rotation change.
        ///     - OrientToCameraViewDirection: Rotates the character towards the camera's current view direction (eg: forward
        ///     vector), using rotationRate as the rate of rotation change.
        ///     OrientWithRootMotion:          Append root motion rotation to Character's rotation.
        /// </summary>
        public virtual void SetRotationMode(RotationMode rotationMode) => _rotationMode = rotationMode;

        /// <summary>
        ///     Returns the Character's current rotation mode.
        /// </summary>
        public virtual RotationMode GetRotationMode() => _rotationMode;

        /// <summary>
        ///     The Character's current up vector.
        ///     NOTE: This can be different from this.transform.up.
        /// </summary>
        public virtual Vector3 GetUpVector() => characterMovement.rotation * Vector3.up;

        /// <summary>
        ///     The Character's current right vector.
        ///     NOTE: This can be different from this.transform.right.
        /// </summary>
        public virtual Vector3 GetRightVector() => characterMovement.rotation * Vector3.right;

        /// <summary>
        ///     The Character's current forward vector.
        ///     NOTE: This can be different from this.transform.forward.
        /// </summary>
        public virtual Vector3 GetForwardVector() => characterMovement.rotation * Vector3.forward;

        /// <summary>
        ///     The Character's current velocity.
        /// </summary>
        public virtual Vector3 GetVelocity() => characterMovement.velocity;

        /// <summary>
        ///     Update the Character's velocity.
        /// </summary>
        public virtual void SetVelocity(Vector3 velocity) => characterMovement.velocity = velocity;
        ...
        /// </summary>
        public virtual float GetSpeed() => characterMovement.velocity.magnitude;

        /// <summary>
        ///     The Character's current movement mode.
        /// </summary>
        public virtual MovementMode GetMovementMode() => _movementMode;
        ...
        /// </summary>
        public virtual bool WasOnGround() => characterMovement.wasOnGround;

        /// <summary>
        ///     Was the Character on walkable ground the last frame ?
        /// </summary>
        public virtual bool WasOnWalkableGround() => characterMovement.wasOnWalkableGround;

        /// <summary>
        ///     Is the Character on ground ?
        /// </summary>
        public virtual bool IsOnGround() => characterMovement.isOnGround;

        /// <summary>
        ///     Is the Character on walkable ground ?
        /// </summary>
        public virtual bool IsOnWalkableGround() => characterMovement.isOnWalkableGround;

        /// <summary>
        ///     Returns true if the Character's movement mode is None (eg: is disabled).
        /// </summary>
        public virtual bool IsDisabled() => _movementMode == MovementMode.None;

        /// <summary>
        ///     Returns true if the Character is in the 'Walking' movement mode (eg: on walkable ground).
        /// </summary>
        public virtual bool IsWalking() => _movementMode == MovementMode.Walking;

        /// <summary>
        ///     Returns true if currently falling, eg: on air (not flying) or in not walkable ground.
        /// </summary>
        public virtual bool IsFalling() => _movementMode == MovementMode.Falling;

        /// <summary>
        ///     Returns true if currently swimming (moving through a water volume).
        /// </summary>
        public virtual bool IsSwimming() => _movementMode == MovementMode.Swimming;

        /// <summary>
        ///     Returns true if currently flying (moving through a non-water volume without resting on the ground).
        /// </summary>
        public virtual bool IsFlying() => _movementMode == MovementMode.Flying;

        /// <summary>
        ///     Returns true if character is actually crouching.
        /// </summary>
        public virtual bool IsCrouching() => _isCrouched;

        /// <summary>
        ///     Is the Character jumping ?
        /// </summary>
        public virtual bool IsJumping() => _isJumping;

        /// <summary>
        ///     Returns the current jump count.
        /// </summary>
        public virtual int GetJumpCount() => _jumpCount;

        /// <summary>
        ///     The maximum speed for current movement mode (accounting crouching / sprinting state).
        /// </summary>
        public virtual float GetMaxSpeed() => _movementMode switch
            {
                MovementMode.Walking when IsCrouching() => maxWalkSpeedCrouched,
                MovementMode.Walking when IsSprinting() => maxWalkSpeed * sprintSpeedMultiplier,
                MovementMode.Walking => maxWalkSpeed,
                MovementMode.Falling => maxWalkSpeed,
        ...
        ///     The ground speed that we should accelerate up to when walking at minimum analog stick tilt.
        /// </summary>
        public virtual float GetMinAnalogSpeed() => _movementMode switch
            {
                MovementMode.Walking => minAnalogWalkSpeed,
                MovementMode.Falling => minAnalogWalkSpeed,
                _ => 0.0f
            };
        ...
        /// </summary>
        public virtual float GetBrakingDeceleration() => _movementMode switch
            {
                MovementMode.Walking => brakingDecelerationWalking,
                MovementMode.Falling =>
                    // Falling,
                    // BUT ON non-walkable ground, bypass braking deceleration to force slide off
                    characterMovement.isOnGround ? 0.0f : brakingDecelerationFalling,
                MovementMode.Swimming => brakingDecelerationSwimming,
                MovementMode.Flying => brakingDecelerationFlying,
                _ => 0.0f,
            };
        ...
        /// </summary>
        public virtual Vector3 GetGravityVector() => gravity;
        ...
        /// <summary>
        ///     Calculates a new velocity for the given state, applying the effects of friction or braking friction and
        ///     acceleration or deceleration.
        ///     Calls GetMaxSpeed(), GetMaxAcceleration(), GetBrakingDeceleration().
        /// </summary>
        protected virtual Vector3 CalcVelocity(Vector3 velocity, Vector3 desiredVelocity, float friction,
            bool isFluid = false)
        {
            ...
            if (isZeroAcceleration || isVelocityOverMax)
            {
                ...
                velocity -= (velocity - desiredMoveDirection * velocity.magnitude) *
                            Mathf.Min(friction * Time.deltaTime, 1.0f);
        ...
        ///     Do not override Character's velocity vertical component (ignores desiredVelocity vertical component) as we want to
        ///     keep the effect of gravity.
        ...
        public virtual bool IsInWater() =>
            characterMovement.physicsVolume && characterMovement.physicsVolume.waterVolume;
        ...
        /// <summary>
        ///     Determines the Character's movement when Swimming through a fluid volume, under the effects of gravity and
        ///     buoyancy.
        ///     Ground-Unconstrained movement with full desiredVelocity (lateral AND vertical) applies gravity but scaled by (1.0f
        ///     - buoyancy).
        /// </summary>
        ...
        protected virtual void Flying(Vector3 desiredVelocity)
        {
            ...
            else
            {
                ...
                characterMovement.velocity = CalcVelocity(characterMovement.velocity, desiredVelocity,
                    0.5f * actualFriction, true);
            }
        }
        ...
        /// </summary>
        public virtual bool IsSprinting() => _sprintButtonPressed;

        /// <summary>
        ///     Request the character to sprint.
        ///     Call this from an input event (such as a button 'down' event).
        /// </summary>
        public virtual void Sprint() => _sprintButtonPressed = true;

        /// <summary>
        ///     Request the character to stop sprinting.
        ///     Call this from an input event (such as a button 'up' event).
        /// </summary>
        public virtual void StopSprinting() => _sprintButtonPressed = false;

        /// <summary>
        ///     Start a jump.
        ///     Call this from an input event (such as a button 'down' event).
        /// </summary>
        public void Jump() => _jumpButtonPressed = true;
        ...
        public virtual bool CanJump()
        {
            ...
            if (_jumpCount != 0) return IsFalling();
            // On first jump,
        ...
        protected virtual void PerformJump()
        {
            ...
            if (!_jumpButtonPressed || IsJumping()) return;
        ...
        /// </summary>
        public virtual void Crouch() => _crouchButtonPressed = true;

        /// <summary>
        ///     Request the Character to stop crouching.
        ///     The request is processed on the next FixedUpdate.
        /// </summary>
        public virtual void StopCrouching() => _crouchButtonPressed = false;

        /// <summary>
        ///     Determines if the Character is able to crouch in its current movement mode.
        ///     Defaults to Walking mode only.
        /// </summary>
        public virtual bool CanCrouch() => IsWalking();
        ...
        protected virtual void Crouching()
        {
            ...
            if (_crouchButtonPressed && !IsCrouching())
            {
                ...
                float clampedCrouchedHeight =
                    Mathf.Clamp(crouchedHeight, characterMovement.capsuleRadius * 2.0f,
                        characterMovement.capsuleHeight);
        ...
        /// <param name="overrideVerticalVelocity">
        ///     If true replace the vertical component of the Character's velocity instead of
        ///     adding to it.
        /// </param>
        /// <param name="overrideLateralVelocity">If true replace the XY part of the Character's velocity instead of adding to it.</param>
        public virtual void LaunchCharacter(Vector3 launchVelocity, bool overrideVerticalVelocity = false,
        ...
        {
            _pendingForces += Forced();
            Vector3 Forced() => forceMode switch
            {
        ...
        /// </summary>
        protected virtual Vector3 ComputeCollisionResponseDisplacement(Vector3 displacement,
            ref MovementHit movementHit)
        {
            if (IsWalking())
            {
        ...
        protected virtual bool CanStepUp(ref RaycastHit hitResult) =>
            ...
            !hitResult.rigidbody || hitResult.rigidbody.isKinematic;
        ...
        protected virtual void OnApplyPushForce(ref RigidbodyHit rigidbodyHit)
        {
            ...
            bool collidedWithChar = rigidbodyHit.rigidbody.TryGetComponent(out ECM2Character otherEcm2Character);
            if (collidedWithChar && !pushForceAffectCharacters)
                return;
            ...
            Vector3 v2 = collidedWithChar ? otherEcm2Character.GetVelocity() : rigidbodyHit.otherVelocity;
            ...
            float m1 = mass;
            float m2 = collidedWithChar ? otherEcm2Character.mass : rigidbodyHit.rigidbody.mass;
            ...
            if (collidedWithChar)
                otherEcm2Character.AddForce(optimizedP * m1 * pushForceScale * n, ForceMode.VelocityChange);
            else
                rigidbodyHit.rigidbody.AddForceAtPosition(optimizedP * m1 * pushForceScale * n, rigidbodyHit.point,
                    ForceMode.VelocityChange);
        }

        /// <summary>
        ///     Apply a downward force when standing on top of non-kinematic physics objects (if applyStandingDownwardForce ==
        ///     true).
        ...
        protected virtual void OnApplyStandingDownwardForce(Rigidbody otherRigidbody)
        {
            ...
            if(!otherRigidbody.gameObject.TryGetComponent(out ECM2Character ecm2Character)) 
                return;
            // Apply downward force
            Vector3 downwardForce = mass * GetGravityVector();
            otherRigidbody.AddForceAtPosition(downwardForce * standingDownwardForceScale,
                characterMovement.groundHit.point);
        }
        ...
        /// <summary>
        ///     Determines if the Character should be rotated with otherRigidbody when standing on top of it (eg: rotating
        ///     platform).
        /// </summary>
        ...
        /// </summary>
        public virtual void SetYaw(float value) => transform.rotation = Quaternion.Euler(0.0f, value, 0.0f);
        ...
        {
            if (isRootMotionControllerNotNull)
        ...
        bool isCameraTransformNull;
        bool isRootMotionControllerNotNull;
        protected float yaw;

        public virtual void SetInputVec(InputAction.CallbackContext ctx)
        {
            if (ctx.performed)
            {
                Vector2 ve2 = ctx.ReadValue<Vector2>();
                if (ve2.y == 0)
                {
                    yaw = ve2.x;
        ...
        protected virtual void UpdateRotation()
        {
            ...
            switch (rotationMode)
            {
                ...
                case RotationMode.OrientToCameraViewDirection when isCameraTransformNull:
                    return;
        ...
        ///     If overriden, must call base method in order to fully initialize the class.
        /// </summary>
        protected virtual void OnOnDestroy() => _characterMovement.RemoveCallbackTarget();
        ...
        ///     If overriden, must call base method in order to fully initialize the class.
        /// </summary>
        protected virtual void OnStart() => SetMovementMode(MovementMode.Walking);

        /// <summary>
        ///     Our FixedUpdate method.
        /// </summary>
        protected virtual void OnFixedUpdate() => PerformMovement();
        ...
        void OnDestroy() => OnOnDestroy();

        void OnEnable() => OnOnEnable();

        void OnDisable() => OnOnDisable();

        void Start()
        {
            isRootMotionControllerNotNull = rootMotionController != null;
            isCameraTransformNull = cameraTransform == null;
            OnStart();
        }
